...
public partial class authentication_required_agent_Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (iData["rGuid"] == null)
        {
            ...
            logInvalidRequestBlockIP(obAuthenticationRequiredStatic.agentRGuidNullIPBlocked);
            /* log return to sender */
            logReturnToSender(obAuthenticationRequiredStatic.agentRGuidNull);
            /* return to sender */
            obAuthenticationRequiredAPI.reTurnToSender(requestIP);
        }
        else if (requestURL == "na") {
            /* log invalid request - block IP */
            logInvalidRequestBlockIP(obAuthenticationRequiredStatic.agentRequestURLNAIPBlocked);
            /* log return to sender */
            logReturnToSender(obAuthenticationRequiredStatic.agentRequestURLNA);
            /* return to sender */
            obAuthenticationRequiredAPI.reTurnToSender(requestIP);
        }
        ...
        else if (iData["rGuid"].ToString().Length == 36)
        {
            ...
            {
                logValidError(obAuthenticationRequiredStatic.agentRGuidValid);
                form1.Visible = true;
            }
            else
            {
                /* log invalid request - block IP */
                logInvalidRequestBlockIP(obAuthenticationRequiredStatic.agentRGuidInvalidIPBlocked);
                /* log return to sender */
                logReturnToSender(obAuthenticationRequiredStatic.agentRGuidInvalid);
                /* return to sender */
                obAuthenticationRequiredAPI.reTurnToSender(requestIP);
            }
        }
        ...
        {
            /* log invalid request - block IP */
            logInvalidRequestBlockIP(obAuthenticationRequiredStatic.agentRGuidInvalidFormatIPBlocked);
            /* log return to sender */
            logReturnToSender(obAuthenticationRequiredStatic.agentRGuidInvalidFormat);
            /* return to sender */
            obAuthenticationRequiredAPI.reTurnToSender(requestIP);
        }                
    }
    ...
    public void logInvalidRequestBlockIP(string iTrackerName) {
        ...
        obReQuestTrackerData.trackerCategory = obReQuestTrackerStatic.frameworkRequestAgentAuthDenied;
